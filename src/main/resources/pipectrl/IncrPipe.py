...
# IncrPipe.py
#=======================================================================
'''We create a simple incrementer pipeline (IncrPipe) to test the PipeCtrl
unit by instantiating it in the control unit of the IncrPipe model. The
IncrPipe model has 5 stages: A, B, C, D, E. First stage of the pipeline is
attached to a TestSource model and the last stage is attached to a TestSink.
Datapath has an incrementer in C,D,E stages. We assume that we can never
originate a stall in B stage. B stage sends the control word to the control
unit based on the transaction it is currently processing.  TestSource
contains the transaction as below:
...
import pclib.rtl.regs as regs
import pclib.rtl.arith as arith
